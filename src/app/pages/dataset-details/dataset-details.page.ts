...
export class DatasetDetailsPage implements OnInit {
  ...
  validateDataset(): void {
    ...
    this.datasetService.validateDataset(this.datasetId).subscribe({
      next: (result: any) => {
        console.log('Raw validation response:', result);
        
        // Normalize the response to match ValidationResponse interface
        const status = (result.status || result.Status || 'unknown').toLowerCase() as 'success' | 'error' | 'warning' | 'failed';
        const validationResult: ValidationResponse = {
          status: status,
          message: result.message || result.Message || 'Validation completed',
          ...
          validationId: result.validationId || result.ValidationId || `val_${Date.now()}`,
          versionId: result.versionId || result.VersionId || '',
          timestamp: new Date().toISOString()
        };
        
        console.log('Normalized validation result:', validationResult);
        ...
        if (this.dataset) {
          this.dataset.isValidated = validationResult.status === 'success';
          this.dataset.validationStatus = validationResult.status;
          this.dataset.validationErrors = validationResult.errorCount;
        }
        
        // Show success message and open results modal
        if (validationResult.status === 'success') {
          this.toastService.presentToast('success', 'Dataset validation passed', 3000);
        } else {
          this.toastService.presentToast(
            validationResult.status === 'warning' ? 'warning' : 'error', 
            `Validation completed with ${validationResult.errorCount} ${validationResult.errorCount === 1 ? 'error' : 'errors'}`,
            3000
          );
        }
        
        // Show the validation results in a modal
      ...
      error: (error) => {
        ...
        this.toastService.presentToast('error', this.validationError || 'Unknown error', 4000);
        
        // Create a validation result with error status
        this.validationResult = {
          status: 'error',
          message: this.validationError || 'Unknown error',
          errorCount: 1,
          errorLines: [],
          errors: [{
            rowNumber: 0,
  ...
  /**
   * Loads the preview data for the dataset
   * @param datasetId The ID of the dataset to load
   */
  private loadDatasetPreview(datasetId: string): void {
    this.datasetService.getDatasetPreview(datasetId).subscribe({
      next: (previewResponse: any) => {
        // Assume previewResponse has headers and data arrays
        this.previewData = {
          headers: previewResponse.headers || [],
          data: previewResponse.data || [],
          schema: previewResponse.schema || [],
          totalRows: previewResponse.totalRows || (previewResponse.data ? previewResponse.data.length : 0),
        };
      ...
      error: (error) => {
        ...
        this.previewError = error.message || 'Failed to load dataset preview';
        this.previewReady = false;
        this.isLoading = false;
        // Optionally, load the full dataset as a fallback
        this.loadFullDatasetAsFallback(datasetId);
      }
    });
  }
  ...
  private loadSchema(): void {
    ...
    // Store component reference to avoid 'this' issues
    const component = this;

    // Define the schema loading logic
    const loadSchema = (datasetId: string) => {
      component.datasetOps.getDatasetSchema(datasetId).subscribe({
        next: (schemaResponse: any) => {
          try {
            if (schemaResponse?.fields) {
              // Create a new previewData object with the updated schema
              const updatedPreviewData = {
                ...component.previewData,
                schema: schemaResponse.fields.map((field: any) => ({
                  name: field.name || '',
              ...
              // Update the component state
              component.previewData = updatedPreviewData;

              if (updatedPreviewData.data && updatedPreviewData.data.length > 0) {
                component.updateSampleValuesFromPreviewData();
              }
            } else {
              throw new Error('No schema fields in response');
            }
          } catch (error) {
            console.warn('Error processing schema, falling back to inference', error);
            component.inferSchemaFromPreviewData();
          }
        },
        error: (error: any) => {
