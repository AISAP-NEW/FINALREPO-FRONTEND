...
export class DatasetDetailsPage implements OnInit {
  ...
  pageSize: number = 10;   // Number of preview rows to show (default 10)

  // --- Existing and preview state ---
  // previewData is now defined only once below (remove duplicate)


  // Provide template bindings for headers/data
  get headers(): string[] {
  ...
  private loadDataset(datasetId: string): void {
    this.isLoading = true;
    // Load dataset metadata and column stats
    this.datasetService.getDatasetById(datasetId).subscribe({
      next: (dataset) => {
        this.dataset = dataset;
        this.datasetInfo = dataset; // For template
        // If column statistics are included in dataset object, assign here
        // Only assign columnStats if it exists on the dataset object
        if (dataset && (dataset as any).columnStats) {
          this.columnStats = (dataset as any).columnStats;
        } else {
          this.columnStats = [];
        }
      },
  ...
  loadDatasetPreview(datasetId: string | null): void {
    ...
    this.isLoading = true;
    this.datasetService.getDatasetPreview(datasetId).pipe(
      finalize(() => {
        this.isLoading = false;
      ...
      next: (response) => {
        console.log('Dataset preview response:', response);
        if (response && response.headers && response.data) {
          ...
          // Infer schema from the preview data
          this.inferSchemaFromPreviewData();
        } else {
          console.warn('Unexpected response format:', response);
          this.toastService.presentToast('warning', 'Unexpected data format received', 3000);
  ...
  // Reset preview data to initial state
